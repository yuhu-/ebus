...
class JsonReader {
 public:
   ...
   * Should return true if the field was handled (and the value consumed/advanced), 
   * false otherwise to let the reader skip the value automatically.
   */
   ...
   * found.
   * @note This method resets the reader's internal position and re-parses from the beginning.
   */
   ...
   */
  template <typename T>
  T asNum() const { return ebus::toNum<T>(value_); }

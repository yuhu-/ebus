  ...
  if (!m_open) {
    ...
    if (m_fd < 0 || isatty(m_fd) == 0)
      throw std::runtime_error(
          "An error occurred while opening the ebus device");
    ...
    newSettings.c_lflag &=
        ~(ICANON | ECHO | ECHOE | ISIG);  // non-canonical mode
...
void ebus::Device::recv(uint8_t &byte, const uint8_t sec, const uint16_t nsec) {
  ...
  if (sec > 0 || nsec > 0) {
    int ret;
    struct timespec tdiff = {sec, nsec * 1000L};
    ...
    if (ret == -1)
      throw std::runtime_error(
          "An device error occurred while waiting on ppoll");
    if (ret == 0)
      throw ebus::runtime_warning(
          "A timeout occurred while waiting for incoming data");
  }

  // read byte from device
  ssize_t nbytes = read(m_fd, &byte, 1);
  if (nbytes < 0)
    throw std::runtime_error("An error occurred while reading file descriptor");
  if (nbytes == 0)
    throw ebus::runtime_warning(
        "An EOF occurred while data was being received");
}
...
bool ebus::Device::isValid() {
  ...
  if (ioctl(m_fd, TIOCMGET, &port) == -1) {
    close();
    throw std::runtime_error(
        "The file descriptor of the ebus device is invalid");
  }
